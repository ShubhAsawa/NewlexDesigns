...
const PortfolioGrid = () => {
  return (
    <section className="py-20 bg-gray-100 overflow-hidden">
      ...
      {/* Full-width Responsive Grid (no gaps, no side space) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 w-full">
        {portfolioProjects.map((project) => (
          <motion.div
            key={project.id}
            className="relative w-full h-[300px] overflow-hidden shadow-lg cursor-pointer group"
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.6 }}
          >
            <Link to={`/portfolio/${project.id}`}>
              {/* Project Image - only image zooms on hover */}
              <motion.img
                src={project.thumbnail}
                alt={project.title}
              ...
              {/* Overlay Text */}
              <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4 bg-black/30 transition-all duration-300">
                <motion.h3
                  className="text-2xl md:text-3xl font-bold text-white"
                  initial={{ y: 10, opacity: 0.8 }}
                  animate={{ y: 0, opacity: 1 }}
                ...
                {/* See Projects Button */}
                <motion.button
                  className="mt-4 px-6 py-2 bg-[#E32225] text-white font-semibold hover:bg-black transition-colors"
                  whileHover={{ scale: 1.05 }}
                  transition={{ duration: 0.3 }}
                >
                  See Projects
